...
ensure_web_admin_built() {
    if [[ ! -d "$WEB_ADMIN_DIR" ]]; then
        log_warn "admin/web directory not found, skip frontend build"
        return 0
    fi
...
cmd_start() {
    ...
    if wait_for_http "$ADMIN_URL/healthz" 60 2; then
        ...
        while [[ $attempt -lt 5 ]]; do
            if curl -sf "${ADMIN_URL}/admin" -o /dev/null 2>/dev/null; then
                admin_ok=true
                break
            fi
            sleep 1
        ...
        if [[ "$admin_ok" != true ]]; then
            log_warn "healthz OK but /admin/ is not serving the SPA"
            if grep -q "bind.*8787\|10048\|Address already in use" "$ADMIN_LOG_FILE" 2>/dev/null; then
                log_err "Port ${ADMIN_PORT} may be in use — stop the old process and retry (see docs/rust/troubleshooting.md)"
            else
                log_err "Build admin/web: cd admin/web && npm run build — see docs/rust/troubleshooting.md"
            fi
            cmd_stop
            exit 1
        fi
        ...
        log_err "Admin server failed to start (check ${ADMIN_LOG_FILE})"
        if grep -q "bind.*8787\|10048\|Address already in use" "$ADMIN_LOG_FILE" 2>/dev/null; then
            log_err "Hint: port ${ADMIN_PORT} in use — free it before starting admin"
        fi
        cmd_stop
        exit 1
    fi
