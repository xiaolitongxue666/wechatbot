...
test("web admin shell renders", async ({ page }) => {
  ...
  await page.route("**/admin/api/system-logs/worker**", async (route) => {
    await route.fulfill({
      ...
      body: JSON.stringify({
        ...
        requested_lines: 200,
        returned_lines: 1,
        lines: ["2026-05-19T10:00:00Z bot-001 forward event consumed"],
      }),
    });
  });
  ...
  await expect(page.getByText(/wx_user -> bot-001: hello/)).toBeVisible();
  await expect(page.getByText(/发送请求/)).toBeVisible();
});
